...
def create_legend(
    ...
    if categories:
        ...
        values = [
            (cat_by_idx[idx], encoding[i], None)
...
class Components:
    ...
    def size(self):
        return reduce(
            lambda acc, i: acc + int(self._components[i].used), self._components, 0
        )
...
class Encodings:
    ...
    def is_unique(self, channel):
        ...
        return (
            sum(
                [
                    v.dimension == self.visual[channel].dimension
                    for v in self.visual.values()
                ]
            )
            == 1
        )
