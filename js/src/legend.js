...
function createIcon(visualChannel, encoding, encodingRange, sizePx, fontColor) {
...
export function createLegend(encodings, fontColor, backgroundColor, size) {
  ...
  for (const encodingEntry of sortedEncodings) {
    ...
    const values =
      typeof visualEncoding.values[0][0] === 'number'
        ? [...visualEncoding.values].reverse()
        : visualEncoding.values;

    for (const [value, encodedValue, label] of values) {
      encoding.appendChild(
        createEntry(
          visualChannel,
          formatter(value),
